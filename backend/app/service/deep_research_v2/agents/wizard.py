...
```"""
    ...
    def _save_debug_info(self, raw_code: str, cleaned_code: str, error: Exception = None):
        ...
        runtime_dir = os.getenv("NEV_ASSISTANT_RUNTIME_DIR", os.path.join(os.getcwd(), "runtime"))
        debug_dir = os.path.join(runtime_dir, "codewizard_debug")
    ...
    def _save_debug_log(self, step_name: str, content: str):
        ...
        if not hasattr(self, '_debug_session_dir'):
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            runtime_dir = os.getenv("NEV_ASSISTANT_RUNTIME_DIR", os.path.join(os.getcwd(), "runtime"))
            self._debug_session_dir = os.path.join(runtime_dir, "codewizard_debug", f"session_{timestamp}")
            os.makedirs(self._debug_session_dir, exist_ok=True)
            self.logger.info(f"[CodeWizard] 调试会话目录: {self._debug_session_dir}")

        # 保存步骤日志
        file_path = os.path.join(self._debug_session_dir, f"{step_name}.txt")
