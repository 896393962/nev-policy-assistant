...
```"""
    ...
    async def _research_section(self, state: ResearchState, section: Dict) -> None:
        ...
        ranked_results = self._rank_context_results(all_results, limit=20)

        self.add_message(state, "thought", {
            "agent": self.name,
            "content": f"搜索完成，获得 {len(all_results)} 条结果，筛选 {len(ranked_results)} 条高价值来源进入分析..."
        })
...
```"""
    ...
    async def _execute_search(self, query: str, count: int = 50) -> List[Dict]:
    ...
    def _rank_context_results(self, results: List[Dict], limit: int = 20) -> List[Dict]:
        """
        Rank and trim search results before sending them to the LLM context.

        The ranking favors official policy sources, industry associations,
        research reports, recent pages and unique URLs.
        """
        seen_urls = set()
        unique_results = []
        for result in results:
            url = (result.get("url") or "").strip().rstrip("/")
        ...
        def score(result: Dict) -> float:
            ...
            low_quality_markers = ["论坛", "贴吧", "知乎", "百家号", "搜狐号", "网易号"]

            if any(marker.lower() in text for marker in official_markers):
                value += 4.0
            if any(marker.lower() in text for marker in industry_markers):
                value += 2.0
            if any(marker.lower() in text for marker in low_quality_markers):
                value -= 2.0

            if any(keyword in title for keyword in ["政策", "补贴", "准入", "规划", "充电", "产业链", "技术路线"]):
                value += 1.0
            if "2026" in date:
                value += 1.2
            elif "2025" in date:
        ...
        return sorted(unique_results, key=score, reverse=True)[:limit]
    ...
    async def _analyze_search_results(
        ...
        # 格式化搜索结果
        formatted_results = []
        for i, r in enumerate(results[:20]):  # 最多分析20条
