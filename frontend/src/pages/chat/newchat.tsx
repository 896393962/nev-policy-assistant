...
export default function NewChat() {
  ...
  const currentIndustry = useMemo(() => {
    ...
    return current || industry.industries[0]
  }, [industry.currentIndustryId, industry.industries])

  const currentIndustryName = currentIndustry?.name || '行业助手'
  const currentAppTitle = currentIndustry?.appTitle || currentIndustryName

  // 推荐问题 - 根据行业生成，只要3个
  const recommendQuestions = useMemo(() => {
    if (currentIndustry?.exampleQuestions?.length) {
      return currentIndustry.exampleQuestions.slice(0, 3)
    }
    return [`${currentIndustryName}市场规模`, `${currentIndustryName}主要企业`, `${currentIndustryName}政策解读`]
  }, [currentIndustry, currentIndustryName])
  ...
  return (
    <div className={styles['newchat-page']}>
      <div className={styles['newchat-page__header']}>
        {query.get('title') || currentAppTitle}
      </div>
